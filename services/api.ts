...
export const api = {
    ...
    database: {
        getTables: async (siteId: string) => {
            return fetchWithMockFallback(
                async () => {
                    const res = await fetch(`${API_URL}/database/${siteId}/tables`, { headers: getAuthHeaders() });
                    return handleResponse(res);
                },
                async () => { await delay(300); return []; }
            );
        },
        getTableStructure: async (siteId: string, tableName: string) => {
            return fetchWithMockFallback(
                async () => {
                    const res = await fetch(`${API_URL}/database/${siteId}/tables/${tableName}/structure`, { headers: getAuthHeaders() });
                    return handleResponse(res);
                },
                async () => { await delay(300); return []; }
            );
        },
        getTableData: async (siteId: string, tableName: string, page: number = 1, limit: number = 50) => {
            return fetchWithMockFallback(
                async () => {
                    const res = await fetch(`${API_URL}/database/${siteId}/tables/${tableName}/data?page=${page}&limit=${limit}`, { headers: getAuthHeaders() });
                    return handleResponse(res);
                },
                async () => { await delay(300); return { data: [], pagination: { page, limit, total: 0 } }; }
            );
        },
        query: async (siteId: string, query: string) => {
            return fetchWithMockFallback(
                async () => {
                    const res = await fetch(`${API_URL}/database/${siteId}/query`, {
                        method: 'POST',
                        headers: getAuthHeaders(),
                ...
                async () => { await delay(300); return { results: [] }; }
            );
        }
    },
