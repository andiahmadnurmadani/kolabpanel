            ...
            <div className="md:col-span-1 flex flex-col gap-4">
                 <Card className="h-full flex flex-col">
                     <div className="flex justify-between items-center mb-4">
                         ...
                         <button onClick={() => setIsCreating(true)} className="p-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors"><Plus className="w-4 h-4" /></button>
                     </div>
                     {isCreating && (
                         <form onSubmit={handleCreateTicket} className="mb-4 p-3 bg-slate-50 rounded-lg border border-slate-200">
                             ...
                             <div className="flex justify-end gap-2">
                                 <button type="button" onClick={() => setIsCreating(false)} className="text-xs text-slate-500 hover:text-slate-800">Cancel</button>
                     ...
                     <div className="flex-1 overflow-y-auto pr-2 space-y-2">
                         {tickets.map(ticket => (
                             <div key={ticket.id} onClick={() => setSelectedTicketId(ticket.id)} className={`p-3 rounded-lg border cursor-pointer transition-all ${selectedTicketId === ticket.id ? 'bg-indigo-50 border-indigo-200 ring-1 ring-indigo-200' : 'bg-white border-slate-100 hover:border-slate-300'}`}>
                                 <div className="flex justify-between items-start mb-1"><span className="font-medium text-sm text-slate-800 truncate pr-2">{ticket.subject}</span><StatusBadge status={ticket.status} /></div>
                                 <div className="flex justify-between items-center text-xs text-slate-500"><span>{new Date(ticket.lastMessageAt).toLocaleDateString()}</span><span>ID: #{ticket.id.slice(-4)}</span></div>
                             </div>
                         ))}
                         {tickets.length === 0 && !isCreating && <div className="text-center py-8 text-slate-400 text-sm">No tickets found.</div>}
                     </div>
                 </Card>
            </div>
            {/* Chat Area */}
                    ...
                    {selectedTicket ? (
                        <>
                            <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                                <div><h3 className="font-bold text-slate-800">{selectedTicket.subject}</h3><p className="text-xs text-slate-500">Ticket ID: {selectedTicket.id}</p></div><StatusBadge status={selectedTicket.status} />
                            </div>
                            ...
                            {selectedTicket.status === 'OPEN' ? (
                                ...
                                <div className="p-4 border-t border-slate-100 bg-slate-50 text-center text-sm text-slate-500">This ticket is closed.</div>
                            )}
                        </>
                    ) : (
