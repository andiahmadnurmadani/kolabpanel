...
<head>
    ...
    <title>Daftar Kampanye - DonGiv Admin</title>
    ...
    <div class="flex-1 p-8 overflow-auto">
        <header class="mb-8 fade-in">
            <div class="flex justify-between items-center">
                <div>
                    <h2 class="text-3xl font-bold text-gray-800">Daftar Kampanye</h2>
                    <p class="text-gray-600">Kelola kampanye donasi untuk membantu sesama</p>
                </div>
                <a href="{{ route('admin.campaigns.create') }}" class="btn-primary py-3 px-6 rounded-xl text-white font-semibold relative overflow-hidden">
                    <span class="relative z-10"><i class="fas fa-plus mr-2"></i>Tambah Kampanye</span>
        ...
        @endif
        @if (session('error'))
            <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-xl mb-6 fade-in">
                {{ session('error') }}
            </div>
        @endif
        ...
        <div class="card p-6 rounded-2xl fade-in">
            @if (empty($campaigns))
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-500">Buat kampanye donasi pertama Anda untuk mulai membantu sesama</p>
                    <a href="{{ route('admin.campaigns.create') }}" class="inline-block mt-4 bg-blue-100 hover:bg-blue-200 text-blue-700 font-semibold py-2 px-6 rounded-lg transition-colors">
                        Buat Kampanye
                    </a>
                </div>
            @else
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-blue-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-blue-700 uppercase tracking-wider">Target</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-blue-700 uppercase tracking-wider">Status</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-blue-700 uppercase tracking-wider">Dibuat</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($campaigns as $campaign)
                                <tr class="hover:bg-blue-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-600">Rp{{ number_format($campaign->target_amount, 0, ',', '.') }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
